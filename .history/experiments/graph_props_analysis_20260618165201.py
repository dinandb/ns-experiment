...
for _, row in df.iterrows():
    ...
    if (graph_size) in no_edges_per_graph_size_df:
        no_edges_per_graph_size_df[(run, graph_size)].extend(list_no_edges)
        no_concomps_per_graph_size_df[(run, graph_size)].extend(list_no_concomps)
    else:
        no_edges_per_graph_size_df[(run, graph_size)] = list(list_no_edges)
        no_concomps_per_graph_size_df[(run, graph_size)] = list(list_no_concomps)

# list mean no_edge per (run, graph_size)

# list mean no_concomps per (run, graph_size)
