...
modularity_df = {} # a key = (algorithm, graph_size)

for _, row in df.iterrows():
    n_added = increments[row["n_graphs"]]
    algorithm = row["algorithm"]
    graph_size = row["graph_size"]
    modularities = row["optimal_modularities"]
    # if modularity_df.keys().contains( (algorithm, graph_size) )
    #   modularity_def[(algorithm, graph_size)].extend( modularities )
    # else
    #   modularities_def[(algorithm, graph_size)] = modularities

pandas_mod_df = pd.DataFrame(modularity_df)
