...
modularity_df = {} # key = graphsize, run

for _, row in df.iterrows():
    if not row.keys().contains("all_numbers_of_edges"):
        continue
    # group no_edges,no_concomps per (dendrogram (run), graph_siz, list mean deg per run if interesting diff


    # group no_edges,no_concomps 
