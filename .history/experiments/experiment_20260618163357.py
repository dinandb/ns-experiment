...
def run_and_compare(n_nodes: int, dend: Dendrogram, n_graphs: int, additional_n_graphs: int, coph_sum, algorithm) -> tuple[float, np.array, np.array, np.array]:
    ...
    return nhmi_score, optimal_modularities, elapsed_times, coph_sum, numbers_of_edges, numbers_of_conn_comps
...
def run_single_experiment(graph_size, run_i, alg):
    ...
    for i, n_graphs in enumerate(N_GRAPHS_LIST):
        ...
        # ontvang lijsten van no_edges, no_components, groepeer van deze run per graph size
        # aan het einde van deze functie voeg toe aan records (key=(graph_size, run_i) -> lijst[int] = no_edges, lijst[int] = no_comps)
        ...
        records.append({
            ...
            "timings": elapsed_times,
        })
        print(f"reported: graph_size {graph_size}, run {run_i},  {alg_name} n_graphs={n_graphs}: nhmi={nhmi_score:.4f}, mod={optimal_modularities} timings=({elapsed_times}s)")
    records.append({
        "graph_size": graph_size,
        "run": run_i,
        "all_numbers_of_edges": all_numbers_of_edges,
        "all_numbers_of_concomps": all_numbers_of_concomps,
    })
    return records
