...
for (alg, gs, run), ng_dict in entry_index.items():
    mod_vals  = [ng_dict[100]["avg_modularity"] for ng in N_GRAPHS_VALS]
    nhmi_vals = [ng_dict[ng]["nhmi_score"]      for ng in N_GRAPHS_VALS]
    vectors[(alg, gs, run)] = np.array(mod_vals + nhmi_vals)
