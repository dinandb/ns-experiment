...
def run_and_compare(n_nodes: int, dend: Dendrogram, n_graphs: int, additional_n_graphs: int, coph_sum, algorithm) -> tuple[float, float]:
    ...
    return nhmi_score, optimal_modularities, coph_sum
...
def run_single_experiment(graph_size, run_i, alg):
    ...
    for i, n_graphs in enumerate(N_GRAPHS_LIST):
        ...
        print(f"reported: graph size {graph_size}, run {run_i},  {alg_name} n_graphs={n_graphs}: nhmi={nhmi_score:.4f}, mod={optimal_modularities:.4f} ({elapsed:.2f}s)")
