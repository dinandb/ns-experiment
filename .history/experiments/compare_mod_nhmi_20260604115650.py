...
# read, for each pair of algorithms A, B, graph_size n, run i:
    ...
    # so the coefficient should be bigger when both e_A[avg_mod] > e_B[avg_mod] and e_A[nhmi_score] > e_B[nhmi_score], and also
    # when we see < both times. 

    # use spearman's rank correlation coefficient for this.
    # comment on the correlation coefficient usage here.